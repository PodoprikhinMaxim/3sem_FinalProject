...
public class HostJob extends Configured implements Tool {
    public static class HostMapper extends Mapper<LongWritable, Text, Text, Text> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            //if (links_showed.length > max) {
            //    System.out.println(arr[1]);
            //    max = links_showed.length;
            ...
            if (arr.length == 4) {
                ...
                for(String link: links_showed) {
                    //if(link.contains("narodstory.net/mif-drevney-grecii.php?id=116")) {
                    ...
                    //context.write(new Text("http"+link),new Text(""+pos_+"\t"+tmp+"\t"+flag_first+"\t"+flag_last+"\t"+time_watch+"\t"+linksExtractor.ids.get(link)));
                ...
                for(String link: links_showed) {
                    ...
                    //String id = linksExtractor.ids.get(link);
                    //pos_ += 1;
                    //context.write(new Text("http"+link), new Text(""+pos_+"\t"+tmp+"\t"+flag_first+"\t"+flag_last+"\t"+time_watch+"\t"+linksExtractor.ids.get(link)));
        ...
        protected void reduce(Text link, Iterable<Text> nums, Context context) throws IOException, InterruptedException {
            ...
            for (Text i: nums) {
                ...
                if(tmp[1].equals("-1")) {
                    ...
                    //Meantime_sum += Double.parseDouble(tmp[4]);
